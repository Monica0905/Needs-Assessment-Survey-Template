...
# Install the needed packages if not installed yet
if(!requireNamespace("dplyr")) install.packages("dplyr", repos = "https://cloud.r-project.org")
if(!requireNamespace("tidytext")) install.packages("tidytext", repos = "https://cloud.r-project.org")
if(!requireNamespace("here")) install.packages("here", repos = "https://cloud.r-project.org")
if(!requireNamespace("ggplot2")) install.packages("ggplot2", repos = "https://cloud.r-project.org")
if(!requireNamespace("stringr")) install.packages("stringr", repos = "https://cloud.r-project.org")
...
needs_full <- read.csv("../Data/DS3 Needs Assessment_April 16.csv")
```
...
# according to order of the checkbox
    need_freq = case_when(
...
write.csv(stat, file = "../Data/Statistical Methodology Needs.csv", row.names = F)
save(stat, file = "../Data/Statistical Methodology Needs.RDATA")
```
...
write.csv(soft, file = "../Data/Software Engineering Needs.csv", row.names = F)
save(soft, file = "../Data/Software Engineering Needs.RDATA")
```
...
write.csv(other, file = "../Data/Other Questions.csv", row.names = F)
save(other, file = "../Data/Other Questions.RDATA")
```
...
# Analyis (just some thoughts...)


```{r}
#table(stat$stat_need)

#research_design <- stat %>%
  #filter(stat_need == "research design")
#apply(research_design[, -1], 2, table)

#tapply(stat$need_freq, stat$stat_need, table)


# needs freqency
need_freq_cnt <- stat %>%
  group_by(stat_need, need_freq) %>%
  summarise(cnt = n())

ggplot(need_freq_cnt, aes(x = stat_need, y = cnt, fill = need_freq)) +
  geom_bar(stat="identity") +
  xlab("Statistical Methodology") +
  ylab("Number of Respondens") +
